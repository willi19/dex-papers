// Twisting Lids Off with Two Hands — review v6 (2026-09-04, sparsity pass; current)
// Written under the rewritten paper-reading skill: field sparsity plus a cross-field
// redundancy check. coreInsight is one paragraph and absorbs the novelty sentence,
// `output` is gone, pipeline drops from 8 steps to 4, methodDetails from 4 items to 1,
// figures from 3 to 2 with two-part captions, evidence from 9 items to 5. Reproduction
// detail (PPO hyperparameters, network sizes, randomisation ranges, object dimensions)
// is out. No number was invented; 65% of v5's size.
...
window.DETAILED_PAPER_SUMMARIES["twisting-lids-v6"] = {
    ...
    problem: [
      ...
      "For tasks this contact-rich the field reaches for demonstrations, which works with parallel jaws and stalls with two multi-fingered hands, because high-quality bimanual multi-finger demonstration data does not exist and the available gloves, the exoskeletons and the vision-based retargeting rigs each give up latency, accuracy, dexterity or cost. Reward recipes carried over from single-hand in-hand reorientation describe one hand rotating a single-part rigid body, so they say nothing about which surface each finger belongs on.",
    ...
    researchNotes: [
      ...
      "<strong>Open question:</strong> the reference point sets are placed by a person. If a segmentation model supplied them at training time, the contact prior would stop being an object-specific annotation and become a procedure.",
